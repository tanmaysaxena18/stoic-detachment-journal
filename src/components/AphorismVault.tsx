...
import { Pillar, Quote, Post } from "../types";
import { Search, Copy, Check, ArrowRight, Plus, Wind, Play, Pause, RefreshCw, PenTool, User, Layers, Volume2, Image as ImageIcon, Compass, Trophy, Sparkles, BookOpen } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import SoundscapePlayer, { speakQuoteOrPost } from "./SoundscapePlayer";
import MultiQuoteImporter from "./MultiQuoteImporter";
import LatestPostsStream from "./LatestPostsStream";
import StoicOracle from "./StoicOracle";
import DailyStoicChallenge from "./DailyStoicChallenge";
import QuoteCardStudioModal from "./QuoteCardStudioModal";
...
export default function AphorismVault({
  onSelectQuote,
  quotes,
  savedPosts = [],
  onAddQuote,
  ...
  // Sub-view Tab State
  const [activeVaultTab, setActiveVaultTab] = useState<"vault" | "latest_posts" | "multi_add" | "oracle" | "challenge">("vault");

  // Visual Card Studio Modal State
  const [cardStudioQuote, setCardStudioQuote] = useState<{ text: string; author?: string; category?: string } | null>(null);
  ...
  return (
    <div id="aphorism-vault-container" className="space-y-6">
      ...
      {/* Intro section & Sub-navigation Header */}
      <div className="bg-zinc-900 border border-zinc-800 p-6 md:p-8 rounded-2xl relative overflow-hidden space-y-4">
        ...
        <div>
          <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-emerald-400 block mb-1">
            System Overview & Archive
          </span>
          <h2 className="text-xl font-sans font-light uppercase tracking-wider text-zinc-100">
            The Aphorism Vault & Interactive Suite
          </h2>
          <p className="text-xs text-zinc-400 max-w-3xl leading-relaxed font-sans mt-1">
            Explore critical observations, review your latest published posts, batch import quote packs, or run real-life situations through the Stoic Dilemma Oracle.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-zinc-800/80">
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2 text-xs font-mono rounded-xl transition ${
          ...
          >
            ...
            Archive ({quotes.length} Quotes)
          </button>

          <button
            onClick={() => setActiveVaultTab("latest_posts")}
            className={`flex items-center gap-2 px-3.5 py-2 text-xs font-mono rounded-xl transition ${
              activeVaultTab === "latest_posts"
                ? "bg-zinc-100 text-zinc-950 font-medium shadow-sm"
                : "bg-zinc-950 text-zinc-400 hover:text-zinc-200 border border-zinc-800"
            }`}
          >
            <PenTool className="w-3.5 h-3.5" />
            My Latest Posts ({savedPosts.length})
          </button>

          <button
            onClick={() => setActiveVaultTab("multi_add")}
            className={`flex items-center gap-2 px-3.5 py-2 text-xs font-mono rounded-xl transition ${
              activeVaultTab === "multi_add"
                ? "bg-zinc-100 text-zinc-950 font-medium shadow-sm"
                : "bg-zinc-950 text-zinc-400 hover:text-zinc-200 border border-zinc-800"
            }`}
          >
          ...
          <button
            onClick={() => setActiveVaultTab("oracle")}
            className={`flex items-center gap-2 px-3.5 py-2 text-xs font-mono rounded-xl transition ${
              activeVaultTab === "oracle"
                ? "bg-zinc-100 text-zinc-950 font-medium shadow-sm"
                : "bg-zinc-950 text-zinc-400 hover:text-zinc-200 border border-zinc-800"
            }`}
          >
          ...
          <button
            onClick={() => setActiveVaultTab("challenge")}
            className={`flex items-center gap-2 px-3.5 py-2 text-xs font-mono rounded-xl transition ${
              activeVaultTab === "challenge"
                ? "bg-zinc-100 text-zinc-950 font-medium shadow-sm"
                : "bg-zinc-950 text-zinc-400 hover:text-zinc-200 border border-zinc-800"
            }`}
          >
      ...
      {/* VIEW 1: MY LATEST POSTS FEED */}
      {activeVaultTab === "latest_posts" && (
        <LatestPostsStream
          posts={savedPosts}
          onNavigateToCurator={onNavigateToCurator}
          onOpenCardStudio={(text, author) => setCardStudioQuote({ text, author })}
        />
      )}

      {/* VIEW 2: MULTI QUOTE BATCH IMPORTER */}
      {activeVaultTab === "multi_add" && (
        <MultiQuoteImporter
          onAddQuotes={(newQuotes) => {
            if (onAddQuotes) onAddQuotes(newQuotes);
            else if (onAddQuote) newQuotes.forEach((q) => onAddQuote(q));
          }}
        />
      )}

      {/* VIEW 3: STOIC DILEMMA ORACLE */}
      {activeVaultTab === "oracle" && <StoicOracle />}

      {/* VIEW 4: DAILY STOIC CHALLENGE */}
      {activeVaultTab === "challenge" && <DailyStoicChallenge />}

      {/* VIEW 0: MAIN SYSTEM VAULT */}
      {activeVaultTab === "vault" && (
        <>
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-zinc-800/50 mt-6 pt-4">
                    <div className="flex items-center gap-2">
                      ...
                      {/* Speech Reader */}
                      <button
                        onClick={() => speakQuoteOrPost(quote.text)}
                        className="p-1 hover:bg-zinc-800 text-zinc-500 hover:text-zinc-200 rounded transition"
                      ...
                      <button
                        onClick={() =>
                          setCardStudioQuote({
                            text: quote.text,
                            author: quote.author || "Stoic Observation",
                            category: quote.category
                          })
                        }
                        className="p-1 hover:bg-zinc-800 text-zinc-500 hover:text-zinc-200 rounded transition"
                        title="Export Visual Quote Card"
                      >
      ...
      {/* Visual Quote Card Studio Modal */}
      {cardStudioQuote && (
        <QuoteCardStudioModal
          isOpen={!!cardStudioQuote}
          quoteText={cardStudioQuote.text}
          author={cardStudioQuote.author}
          category={cardStudioQuote.category}
          onClose={() => setCardStudioQuote(null)}
        />
      )}
    </div>
  );
